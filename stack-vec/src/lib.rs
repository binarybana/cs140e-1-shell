...
impl<'a, T: 'a> StackVec<'a, T> {
    ...
    pub fn with_len(storage: &'a mut [T], len: usize) -> StackVec<'a, T> {
        if len > storage.len() {
            panic!("Creation of StackVec failed with len > storage");
        }
    ...
    /// Obtains an iterator of the current vector.
...
impl<'a, T: 'a> Index<usize> for StackVec<'a, T> {
    ...
    fn index(&self, index: usize) -> &Self::Output {
        if self.len == 0 || index > self.len - 1 {
            panic!("Out of bounds access on StackVec");
        }
        &self.storage[index]
    }
}
...
// FIXME: Implement `Index`, `IndexMut`, and `IntoIterator` for `StackVec`.
